...
if __name__ == '__main__':
    ...
    for subject in subjects:
        ...
        # frequencies to consider; HRTF azimuths 
        freqs = np.arange(250, 1251, 25) 
        ...
        for i, az in tqdm(enumerate(azims)):
            # convolving the sound signal with the HRTF
            convolved_l = Sound(convolve(sound, hrir_l[i]), samplerate=Sf*Hz)
            convolved_r = Sound(convolve(sound, hrir_r[i]), samplerate=Sf*Hz)

            # passing the convolved sound through the cochlear filter
            cochlear_l = Gammatone(convolved_l, freqs).process() 
            cochlear_r = Gammatone(convolved_r, freqs).process()

            # iterate over cochelar filter center freqs
            for cl, cr, j in zip(cochlear_l.T, cochlear_r.T, range(len(freqs))):
                # employ hilbert transform to get the phase difference between left and right filtered signals  
                transformed = hilbert(np.stack((cl, cr)), axis=1)

                ipd = wrapToPi(np.diff(np.angle(transformed), axis=0))[0, :]
                ipd = ipd[trim:-trim]

                # get the mean ipd value and its standard deviation
        ...
        # unwrapping the IPD values, also managing phase ambiguity
        for i in range(len(freqs)):
    ...
    for i in range(len(freqs)):
        ...
        # consider only the FI values between the [-pi,pi] IPD domain
